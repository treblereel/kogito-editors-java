package org.uberfire.ext.widgets.table.client.resources;

import org.gwtproject.dom.client.StyleInjector;
import org.gwtproject.i18n.shared.cldr.LocaleInfo;
import org.gwtproject.resources.client.ImageResource;
import org.gwtproject.resources.client.ResourcePrototype;
import org.gwtproject.resources.client.impl.ImageResourcePrototype;
import org.gwtproject.safehtml.shared.UriUtils;

public class UberfireSimplePagerResources_default_InlineClientBundleGenerator implements UberfireSimplePagerResources {
  private static UberfireSimplePagerResources_default_InlineClientBundleGenerator _instance0 = new UberfireSimplePagerResources_default_InlineClientBundleGenerator();
  private void simplePagerFastForwardInitializer() {
    simplePagerFastForward = new ImageResourcePrototype(
            "simplePagerFastForward",
            UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl : externalImage),
            0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerFastForwardInitializer {
    static {
      _instance0.simplePagerFastForwardInitializer();
    }
    static ImageResource get() {
      return simplePagerFastForward;
    }
  }
  public ImageResource simplePagerFastForward() {
    return simplePagerFastForwardInitializer.get();
  }
  private void simplePagerFastForwardDisabledInitializer() {
    simplePagerFastForwardDisabled = new ImageResourcePrototype(
      "simplePagerFastForwardDisabled",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl0 : externalImage0),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerFastForwardDisabledInitializer {
    static {
      _instance0.simplePagerFastForwardDisabledInitializer();
    }
    static ImageResource get() {
      return simplePagerFastForwardDisabled;
    }
  }
  public ImageResource simplePagerFastForwardDisabled() {
    return simplePagerFastForwardDisabledInitializer.get();
  }
  private void simplePagerFirstPageInitializer() {
    simplePagerFirstPage = new ImageResourcePrototype(
      "simplePagerFirstPage",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl1 : externalImage1),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerFirstPageInitializer {
    static {
      _instance0.simplePagerFirstPageInitializer();
    }
    static ImageResource get() {
      return simplePagerFirstPage;
    }
  }
  public ImageResource simplePagerFirstPage() {
    return simplePagerFirstPageInitializer.get();
  }
  private void simplePagerFirstPageDisabledInitializer() {
    simplePagerFirstPageDisabled = new ImageResourcePrototype(
      "simplePagerFirstPageDisabled",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl2 : externalImage2),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerFirstPageDisabledInitializer {
    static {
      _instance0.simplePagerFirstPageDisabledInitializer();
    }
    static ImageResource get() {
      return simplePagerFirstPageDisabled;
    }
  }
  public ImageResource simplePagerFirstPageDisabled() {
    return simplePagerFirstPageDisabledInitializer.get();
  }
  private void simplePagerLastPageInitializer() {
    simplePagerLastPage = new ImageResourcePrototype(
      "simplePagerLastPage",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl3 : externalImage3),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerLastPageInitializer {
    static {
      _instance0.simplePagerLastPageInitializer();
    }
    static ImageResource get() {
      return simplePagerLastPage;
    }
  }
  public ImageResource simplePagerLastPage() {
    return simplePagerLastPageInitializer.get();
  }
  private void simplePagerLastPageDisabledInitializer() {
    simplePagerLastPageDisabled = new ImageResourcePrototype(
      "simplePagerLastPageDisabled",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl4 : externalImage4),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerLastPageDisabledInitializer {
    static {
      _instance0.simplePagerLastPageDisabledInitializer();
    }
    static ImageResource get() {
      return simplePagerLastPageDisabled;
    }
  }
  public ImageResource simplePagerLastPageDisabled() {
    return simplePagerLastPageDisabledInitializer.get();
  }
  private void simplePagerNextPageInitializer() {
    simplePagerNextPage = new ImageResourcePrototype(
      "simplePagerNextPage",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl5 : externalImage5),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerNextPageInitializer {
    static {
      _instance0.simplePagerNextPageInitializer();
    }
    static ImageResource get() {
      return simplePagerNextPage;
    }
  }
  public ImageResource simplePagerNextPage() {
    return simplePagerNextPageInitializer.get();
  }
  private void simplePagerNextPageDisabledInitializer() {
    simplePagerNextPageDisabled = new ImageResourcePrototype(
      "simplePagerNextPageDisabled",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl6 : externalImage6),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerNextPageDisabledInitializer {
    static {
      _instance0.simplePagerNextPageDisabledInitializer();
    }
    static ImageResource get() {
      return simplePagerNextPageDisabled;
    }
  }
  public ImageResource simplePagerNextPageDisabled() {
    return simplePagerNextPageDisabledInitializer.get();
  }
  private void simplePagerPreviousPageInitializer() {
    simplePagerPreviousPage = new ImageResourcePrototype(
      "simplePagerPreviousPage",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl7 : externalImage7),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerPreviousPageInitializer {
    static {
      _instance0.simplePagerPreviousPageInitializer();
    }
    static ImageResource get() {
      return simplePagerPreviousPage;
    }
  }
  public ImageResource simplePagerPreviousPage() {
    return simplePagerPreviousPageInitializer.get();
  }
  private void simplePagerPreviousPageDisabledInitializer() {
    simplePagerPreviousPageDisabled = new ImageResourcePrototype(
      "simplePagerPreviousPageDisabled",
      UriUtils.fromTrustedString(LocaleInfo.getCurrentLocale().isRTL() ?externalImage_rtl8 : externalImage8),
      0, 0, 19, 19, false, false
    );
  }
  private static class simplePagerPreviousPageDisabledInitializer {
    static {
      _instance0.simplePagerPreviousPageDisabledInitializer();
    }
    static ImageResource get() {
      return simplePagerPreviousPageDisabled;
    }
  }
  public ImageResource simplePagerPreviousPageDisabled() {
    return simplePagerPreviousPageDisabledInitializer.get();
  }
  private void simplePagerStyleInitializer() {
    simplePagerStyle = new org.uberfire.ext.widgets.table.client.UberfireSimplePager.Style() {
      private boolean injected;
      public boolean ensureInjected() {
        if (!injected) {
          injected = true;
          StyleInjector.inject(getText());
          return true;
        }
        return false;
      }
      public String getName() {
        return "simplePagerStyle";
      }
      public String getText() {
        return (".GIYSKH5CKN{padding:" + ("4px"+ " " +"8px")  + ";text-align:" + ("center")  + ";}.GIYSKH5CJN{padding:" + ("4px")  + ";cursor:" + ("pointer")  + ";cursor:" + ("hand")  + ";}");
      }
      public String button() {
        return "GIYSKH5CJN";
      }
      public String pageDetails() {
        return "GIYSKH5CKN";
      }
    }
    ;
  }
  private static class simplePagerStyleInitializer {
    static {
      _instance0.simplePagerStyleInitializer();
    }
    static org.uberfire.ext.widgets.table.client.UberfireSimplePager.Style get() {
      return simplePagerStyle;
    }
  }
  public org.uberfire.ext.widgets.table.client.UberfireSimplePager.Style simplePagerStyle() {
    return simplePagerStyleInitializer.get();
  }
  private static java.util.HashMap<String, ResourcePrototype> resourceMap;
  private static final String externalImage = "data:image/png;base64,";
  private static final String externalImage_rtl = "data:image/png;base64,";
  private static final String externalImage0 = "data:image/png;base64,";
  private static final String externalImage_rtl0 = "data:image/png;base64,";
  private static final String externalImage1 = "data:image/png;base64,";
  private static final String externalImage_rtl1 = "data:image/png;base64,";
  private static final String externalImage2 = "data:image/png;base64,";
  private static final String externalImage_rtl2 = "data:image/png;base64,";
  private static final String externalImage3 = "data:image/png;base64,";
  private static final String externalImage_rtl3 = "data:image/png;base64,";
  private static final String externalImage4 = "data:image/png;base64,";
  private static final String externalImage_rtl4 = "data:image/png;base64,";
  private static final String externalImage5 = "data:image/png;base64,";
  private static final String externalImage_rtl5 = "data:image/png;base64,";
  private static final String externalImage6 = "data:image/png;base64,";
  private static final String externalImage_rtl6 = "data:image/png;base64,";
  private static final String externalImage7 = "data:image/png;base64,";
  private static final String externalImage_rtl7 = "data:image/png;base64,";
  private static final String externalImage8 = "data:image/png;base64,";
  private static final String externalImage_rtl8 = "data:image/png;base64,";
  private static ImageResource simplePagerFastForward;
  private static ImageResource simplePagerFastForwardDisabled;
  private static ImageResource simplePagerFirstPage;
  private static ImageResource simplePagerFirstPageDisabled;
  private static ImageResource simplePagerLastPage;
  private static ImageResource simplePagerLastPageDisabled;
  private static ImageResource simplePagerNextPage;
  private static ImageResource simplePagerNextPageDisabled;
  private static ImageResource simplePagerPreviousPage;
  private static ImageResource simplePagerPreviousPageDisabled;
  private static org.uberfire.ext.widgets.table.client.UberfireSimplePager.Style simplePagerStyle;
  
  public ResourcePrototype[] getResources() {
    return new ResourcePrototype[] {
      simplePagerFastForward(), 
      simplePagerFastForwardDisabled(), 
      simplePagerFirstPage(), 
      simplePagerFirstPageDisabled(), 
      simplePagerLastPage(), 
      simplePagerLastPageDisabled(), 
      simplePagerNextPage(), 
      simplePagerNextPageDisabled(), 
      simplePagerPreviousPage(), 
      simplePagerPreviousPageDisabled(), 
      simplePagerStyle(), 
    };
  }
  public ResourcePrototype getResource(String name) {
      if (resourceMap == null) {
        resourceMap = new java.util.HashMap<String, ResourcePrototype>();
        resourceMap.put("simplePagerFastForward", simplePagerFastForward());
        resourceMap.put("simplePagerFastForwardDisabled", simplePagerFastForwardDisabled());
        resourceMap.put("simplePagerFirstPage", simplePagerFirstPage());
        resourceMap.put("simplePagerFirstPageDisabled", simplePagerFirstPageDisabled());
        resourceMap.put("simplePagerLastPage", simplePagerLastPage());
        resourceMap.put("simplePagerLastPageDisabled", simplePagerLastPageDisabled());
        resourceMap.put("simplePagerNextPage", simplePagerNextPage());
        resourceMap.put("simplePagerNextPageDisabled", simplePagerNextPageDisabled());
        resourceMap.put("simplePagerPreviousPage", simplePagerPreviousPage());
        resourceMap.put("simplePagerPreviousPageDisabled", simplePagerPreviousPageDisabled());
        resourceMap.put("simplePagerStyle", simplePagerStyle());
      }
      return resourceMap.get(name);
  }
}
